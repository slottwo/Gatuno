    ...
    def jumping(self, c=10):
        if self.jumpCount >= -c:
            ...
            if self.jumpCount < 0:
                i = -1
                self.isFalling = True
            self.yPos -= int(i * self.jumpCount ** 2 / 2)
            # In the pygame the possition is a integer number // No pygame a posição deve ser dada em inteiros
            self.jumpCount -= 1
        else:
...
def debugging():
    stats = [
        ...
        font.render("Run: " + str(cat.isRunning), True, (0, 255, 0)),
...
def redraw_game_window():
    ...
    screen.blit(pygame.transform.scale(spritePack[sprite_mode][sprite_act][sprite_index], cat.size),
                (cat.xPos, cat.yPos - 4))  # "-4" por convenção do sprite
...
while runGame:
    ...
    if (keys[pygame.K_LEFT] or keys[pygame.K_RIGHT]) and not (
            keys[pygame.K_LEFT] and keys[pygame.K_RIGHT]):  # Xor Logic Gate: True if just one is True
